...
class TaskManagerCLI:
    ...
    def display_menu(self) -> None:
        ...
        print(center_text("7. Summarize a task"))
    ...
    def summarize_task_handler(self) -> None:
        """Summarize a task description using OpenAI API."""
        print(f"\n{HEADER_COLOR}--- Summarize Task ---{Style.RESET_ALL}")
        task, found = self.find_task_by_name()
        
        if not found:
            return
        
        print(f"\n{INFO_COLOR}Task: {task.name}{Style.RESET_ALL}")
        description = input("Enter a full task description to summarize: ").strip()
        
        if not description:
            print(f"{ERROR_COLOR}Description cannot be empty.{Style.RESET_ALL}")
            return
        
        print(f"\n{INFO_COLOR}Summarizing with OpenAI...{Style.RESET_ALL}")
        
        try:
            summary = summarize_task(description)
            print(f"\n{SUCCESS_COLOR}Summary: {summary}{Style.RESET_ALL}")
        except Exception as e:
            print(f"{ERROR_COLOR}Error summarizing task: {e}{Style.RESET_ALL}")
